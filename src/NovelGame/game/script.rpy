...
label AndrewSelected:
    ...
    Andrew talking1 "Ну что, абитуриентик, как оно? Пора определяться." with dissolve
    show Andrew listening1 with dissolve
    GG "Да я же говорил уже. Стараюсь для себя. Ну давай, поведай мне о своём студенчестве, трекус."
    Andrew talking2 "Ого, как мы заговорили, хех, ладно." with dissolve
    Andrew talking3 "В общем, если ты любишь точные науки, такие как математика и, к сожалению, физика, то тут тебе и место, на юрфаке ты больше будешь учить обществознание, право, лево и, внезапно, латынь." with dissolve
    Andrew talking1 "Вообще про работу back-end'ером нам мало что говорили, так что я тут по большей части агитирую тебя поступать, основываясь на твоих интересах." with dissolve
    Andrew talking4 "Из языков ты можешь выбрать C# или PHP, да даже Java с Python'ом. А вот на B✞✞ писать реквесты к царю батюшке через код не удасться, увы." with dissolve
    show Andrew listening2 with dissolve
    GG "Ого"
    show Andrew listening1 with dissolve
    ...
    menu:
        ...
        "Класс, а чем бэкендр разработчик занимается на практике?":
            $ involvement += 1
            Andrew nerd "Ну, чисто в теории, пишет серверный код, отвечающий за реакцию ресурса на действия пользователя и выдачу информации" with dissolve
            show Andrew listening2 with dissolve
            GG "Huh?"
            Andrew talking3 "Ну, или простыми словами: ты пишешь внутренности сайта." with dissolve
            show Andrew listening1 with dissolve
            GG "А, всё понял."
            Andrew talking4 "Ещё какие-то вопросы?" with dissolve
            show Andrew listening1 with dissolve
...
label KateSelected:
    ...
    Kate talking1 "Привет-привет, сколько лет, сколько зим?" with dissolve
    show Kate listening1 with dissolve
    GG "Привет, да, прошло не мало времени с нашей последней встречи."
    Kate talking3 "Как дела, давай, давай колись." with dissolve
    show Kate listening2 with dissolve
    GG "Да вообще отлично. Я вот хочу узнать некоторые вещи по поводу твоей профессии, расскажешь?"
    Kate talking1 "О-ооо, конечно! Что именно тебе рассказать о фронтэнде? Может его отличиях от бэкенда?" with dissolve
    show Kate listening1 with dissolve
    ...
    Kate talking2 "а в-оот, фронтэнд отличается от бэкенда тем, что тут ты стараешься не для других программистов, а для юзеров. Им вообще не важно написан у тебя сайт в 4 метода или в 44, им важен UI, юзер интрефасе, если так удобно, воооот" with dissolve
    show Kate listening2 with dissolve
    GG "Ого, прикольно!"
    ...
    menu:
        ...
        "Я просто думал пойти в бэкенд, но если не получится, то смогу ли я перейти на фронтенд?":
            $ involvement += 1
            Kate talking3 "Думаю, что сможешь. В конце концов и фронтэнд и бэкенд можно писать на JavaScript, хоть у этих двух прфессий и разные предназначения, думаю, что ты быстро должен будешь освоиться с переходом." with dissolve
            show Kate listening2 with dissolve
            GG "Ясно."
            jump KateSecondSelect
        "Думаю, что я всё понял. Пройдёмся?":
            show Kate greetingsOneHand with dissolve
    ...
    menu KateSecondSelect:
        ...
        "А про условия труда есть, что сказать?":
            $ involvement += 1
            Kate talking2 "Труда? А, всё более менее, есть сроки и в них нужно укладываться. Что немало важно нужно иметь хорошую фантазию и чувство вкуса, чтобы создавать что-то действительно необычное и красивое." with dissolve
            show Kate listening1 with dissolve
            GG "Дак это же буквально я."
            Kate talking3 "Ага, Гослинг ты наш, охотно в это верю.  Надеюсь, это не очень всё занудно?" with dissolve
            show Kate listening1 with dissolve
    ...
    menu KateThirdSelect:
        ...
        "А вот ещё, хоть немного и неприлично, но какая зп у фронтэндера?":
            $ involvement += 1
            Kate talking1 "Сейчас, дай-ка посчитаю... ммм…  в среднем это 80+ хихи" with dissolve
            show Kate listening2 with dissolve
            GG "Немало"
            Kate talking3 "Ага, работка не особо пыльная, но освоиться можно и если что, то у тебя обязательно получится." with dissolve
            show Kate listening1 with dissolve
            GG "Ой, уж прости, но мне в выборе из бэкенда и фронтенда больше импонирует первое, хотя звучало довольно заманчиво."
            Kate talking2 "Я тебя не заставляю, это твой выбор." with dissolve
            show Kate listening2 with dissolve
            GG "Ну на этом я думаю, что по моей теме всё. Ну что, как лето-то хоть проводишь?"
            Kate talking1 "Знаешь..." with dissolve
            jump friendSelectAfterKate
        "Думаю, что я всё понял. Пройдёмся?":
            show Kate greetingsOneHand with dissolve
            "ГГ отлично пообщался с Катей на отвлечённый темы"
...
label OlegSelected:
    ...
    Oleg shakehand1 "Приветствую, как дела?" with dissolve
    show Oleg listening1 with dissolve
    GG "Эм, привет, да нормально, а ты всё уже что ли?"
    Oleg talking2 "Ну, я решил, что как-то не культурно будет за столом общаться, так что своевременно закон-чил. Хорошо, что у тебя всё хорошо, как бы это не звучало." with dissolve
    show Oleg listening1 with dissolve
    GG "О как, ну ладно. Я хотел спрость чем вы там занимаетесь вообще?"
    Oleg talking1 "Сразу к делу перешёл, ну тебя совсем после экзаменов не узнать. Пошли выйдем отсюда, чтобы не мешать другим что ли." with dissolve
    show Oleg listening2 with dissolve
    ...
    window show
    Oleg talking2 "Возвращаясь к твоему вопросу: в общем и целом, всяким. Если тебя конкретно интересует бэ-кенд разработка, то мы пишем и переписываем код для заказчиков." with dissolve
    Oleg talking1 "Вот понадобился кому-то сайт, ну вот просто одностраничник, так и радостно, быстро сделаем, а коли у кого-то правок много и требований, так долго возиться с ним будем." with dissolve
    show Oleg listening2 with dissolve
    ...
    menu:
        ...
        "И всё, ничего больше?":
            $ involvement += 1
            Oleg talking2 "Ну есть и некоторые другие задачи, проверять чужой код, например. Не очень простой труд, честно сказать, нужно быть предельно сконцентрированным и сфокусированным. Если про-пустить что-то важное, то кого-то будут бить, и возможно даже ногами. Ну во всяком случае у нас в конторке так." with dissolve
            show Oleg listening1 with dissolve
            GG "Ну про рукоприкладство… или даже ногоприкладство – это же шутка, верно?"
            Oleg talking1 "А вот, кто знает… Отучишься, придёшь и сам посмотришь =)))" with dissolve
            show Oleg listening1 with dissolve
            GG "Понятно, а ты всё такой же."
            Oleg talking2 "Ага, а минусы будут? Что-нибудь ещё рассказать? Какие-нибудь байки ещё травануть?" with dissolve
            show Oleg listening2 with dissolve
    ...
    menu AndrewSecondSelect:
        ...
        "Наверное, не самый хороший вопрос, но а вот как по зп всё?":
            ...
            Oleg talking1 "Всегда пожалуйста. А теперь давай ты рассказывай, как у тебя лето проходит..." with dissolve
            show Oleg listening2 with dissolve
            "После часа хорошей беседы Олег и гг расходятся, кто домой, а кто дальше работать"
            jump friendSelectAfterOleg
